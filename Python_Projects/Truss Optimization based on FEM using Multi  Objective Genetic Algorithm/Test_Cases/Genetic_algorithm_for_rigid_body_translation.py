'''
Basic FEM for Rigid body Translation
youngs_modulus =70000 Pa; 
density=1 Kg/mm^3; 
max_length_of_bar = 18 
max_height_of_bar = 9
Force in all node = 100
...
input_array=[[0,max_length_of_bar,300,300,300,3,300,300],[0,max_length_of_bar,300,300,300,3,300,300],[0,max_length_of_bar,300,0,0,0,0,0]] # For the specific Problem
...
def draw_line(from_point, to_point, element, areas):
    ...
    plt.plot([x_1,x_2],[y_1,y_2], color='green', linestyle='-', linewidth=0.5*areas, zorder=1)
...
with open('Genetic_Algorithm_FEM_with_excel_1.txt','w') as filename:
        filename.write('Stress are %s\n' %stresses)
        filename.write('Global displacements are %s\n' %X)

plt.savefig('Truss Structure_with_excel_1.jpg')