...
time_step_array=np.linspace(0.01,0.03,1)
...
for index,j in enumerate (coordinate):
    if index<no_of_ele:
        value=coordinate[index]+coordinate[index+1]
    else:
        break

    # print(value)
    coordinates=np.append(coordinates,value/2)
...
# print('Mesh/2',coordinates)

#For plot
print('time',time_step)
u_elastic=((inner_radius)**3*(volumetric_strain)*time_step)/(3*(coordinate)**2)
sigma_rr_elastic=-time_step*(2*E*(volumetric_strain)/(3*(1+poissons_ratio)))*((inner_radius)**3/coordinates**3)#
# print('Exact sigma_rr',sigma_rr_elastic)

#Initial Guess of Displacement for the Newton Raphson Method
u=np.linspace(0,(1/3*time_step*(volumetric_strain)*coordinate[0]),(no_of_ele+1)).reshape(no_of_ele+1,1)
...
# Element Routine
def Element_fn(coordinate,Lambda,mu,tau,sigma_rr):
    ...
    C,new_sigma,sigma_xx=Material_fn(coordinate,global_u,sigma_rr)
...
# #Material Routine
def Material_fn(coordinate,global_u,sigma_xx):
    der_shape_fn=np.array([-1/2,1/2])
    ...
    sigma_xx=np.append(sigma_xx,sigma_trail)
    # print(sigma_xx)
...
for time,tau in  enumerate(time_step_array):
    ...
    while np.linalg.norm(delta_u)>(0.005*np.linalg.norm(u_reduced)) or np.linalg.norm(G_matrix_reduced)>(0.005*np.linalg.norm(F_int_global)):
        ...
        # print(F_int_global)
        for i in range(no_of_ele):
            K_ele,f_int_ele = Element_fn(coordinate,Lamda,mu,tau,sigma_xx)
    ...
    # plt.show()
    # print('Numerical sigma_rr',sigma_xx)
    fig,ax=plt.subplots(nrows=1,ncols=2,figsize=(10,5))
    ax[0].set_title('Displacement_numerical_and Exact_solution')
    ax[0].set_xlabel('Coordinate')
    ax[0].set_ylabel('Displacements')
    ax[0].plot(coordinate,global_u,'b',label='Numerical')
    ax[0].plot(coordinate,u_elastic,'r',label='Exact')
    ax[0].legend(loc='upper right', shadow=True, fontsize='x-large')#title='Numerical and Exact displacement comparison')

    ax[1].set_title('Sigma_rr_numerical_and Exact_solution')
    ax[1].set_xlabel('Coordinate')
    ax[1].set_ylabel('sigma_rr')
    ax[1].plot(coordinates,sigma_rr_elastic,'b',label='exact')
    ax[1].plot(coordinate,sigma_rr,'r',label='Numerical')
    ax[1].legend(loc='best', shadow=True, fontsize='x-large')#title='Numerical and Exact Radial_stress comparison')

    plt.tight_layout()
    plt.savefig('Plots_for_Exact_displacements_Stress_at_elastic_condition.png')