...
#For plot
sigma_rr_ri=[]
u_elastic=((inner_radius)**3*(-volumetric_strain))/(3*(coordinate)**2)
sigma_rr_elastic=-(2*E*(-volumetric_strain)/(3*(1+poissons_ratio)))*((inner_radius)**3/coordinates**3)
...
ax[0,0].plot(coordinate,global_u,'b',label='Numerical')
ax[0,0].plot(coordinate,u_elastic,'r',label='Exact')
ax[0,0].set_title('Global Displacement and Coordinate')
ax[0,0].set_xlabel('Coordinates')
ax[0,0].set_ylabel('Global_displacement')
ax[0,1].plot(coordinate,sigma_rr)
ax[0,1].plot(coordinates,sigma_rr_elastic)
